...
const TitleScene = () => {
  ...
  return (
    <AbsoluteFill style={{ background: colors.bg }}>
      ...
      <SoftOrb x="70%" y="60%" size={500} color="#9b59b6" />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ textAlign: "center", transform: `translateY(${interpolate(progress, [0, 1], [40, 0])}px)`, opacity: interpolate(progress, [0, 1], [0, 1]) }}>
          ...
          <h1 style={{ fontSize: 90, fontWeight: 600, color: colors.textPrimary, margin: 0, letterSpacing: "-2px" }}>
            Edición <span style={{ color: colors.primary }}>Oficina</span>
          </h1>
          <p style={{ fontSize: 28, color: colors.textSecondary, marginTop: 30 }}>Lo que realmente pasa detrás del "estoy en una llamada"</p>
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
    ...
    <AbsoluteFill style={{ background: colors.primary }}>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ textAlign: "center", transform: `translateY(${interpolate(progress, [0, 1], [30, 0])}px)`, opacity: interpolate(progress, [0, 1], [0, 1]) }}>
          ...
          <h2 style={{ fontSize: 72, fontWeight: 600, color: "#fff", margin: 0 }}>Hora del coffee break</h2>
          <p style={{ fontSize: 24, color: "rgba(255,255,255,0.8)", marginTop: 20, marginBottom: 50 }}>Edición Oficina - Para team buildings memorables</p>
          <div style={{ transform: `scale(${pulse})` }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 12, background: colors.bg, padding: "24px 48px", borderRadius: 16 }}>
              <span style={{ fontSize: 28, fontWeight: 600, color: colors.primary }}>Jugar en horario laboral</span>
...
const cardPairs = [
  { black: "Durante la junta de Zoom, mi jefe no sabía que todos podían ver", white: "Su búsqueda de 'cómo fingir que trabajo'", orb: colors.primary },
  { black: "El verdadero motivo por el que siempre llego tarde es", white: "Sentarme en el carro 20 minutos juntando fuerzas para entrar", orb: "#9b59b6" },
  { black: "Recursos Humanos me citó porque encontraron en mi computadora", white: "3,847 memes del jefe dormido en reuniones", orb: colors.primary },
  { black: "Mi estrategia para sobrevivir las reuniones largas es", white: "Poner cara de atención mientras planeo mi renuncia", orb: "#9b59b6" },
  { black: "El secreto mejor guardado de la oficina es", white: "Que el café es descafeinado desde 2019", orb: colors.primary },
  { black: "En el chat corporativo accidentalmente envié", white: "Un audio quejándome del jefe... al jefe", orb: "#9b59b6" },
];

export const AdultPromo5 = () => {
  const { fps } = useVideoConfig();
